...
Description: Removes bloatware apps if they are included in the list and found on the machine
Hint: 
Version: 1.0
Run as: System
Context: 64 Bit
...
foreach ($App in $InstalledPackages) {
    try {
        ...
        $AllAppXProvisionedPackage | Where-Object { $_.DisplayName -eq $($App.Name) } |  Remove-AppxProvisionedPackage -Online | Out-Null
        $out += $App.Name
    }
    catch {
...
if ($out.Count -gt 0) {
    Write-Output "Apps removed: ($($out -join ', '))"
    Exit 0
}
